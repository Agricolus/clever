...
|ping_test.py
|Performs IPv4 ICMP ping tests, and retrieves the physical location of the target server.
...
* Runs ping tests using `ping_test.py`
...
=== ping_test.py — IPv4 ICMP Connectivity Test

The ping_test.py module provides a workflow based on the SIMCOM ping application note for testing IPv4 ICMP connectivity using the active PDP context established with the modem_initializer.py module. This script allows the user to verify basic IP reachability and measure network latency before performing higher-level TCP or HTTP(S) tests.

Key features of this module include:

* Prompts the user to enter a target IPv4 address or use a predefined default server.

* Retrieves and displays the physical location (city, region, country, ISP) of the target server using an external IP geolocation service.

* Sends ICMP Echo Requests using the AT+SNPING4 command through the active PDP context.

* Collects and displays per-packet delay measurements.

* Calculates and reports transmitted, received, and lost packets.

* Logs modem commands and responses for debugging purposes.

This module is intended to confirm basic IP connectivity and latency performance, validating the modem, SIM card, APN configuration, and network attachment before proceeding to TCP or HTTP(S) communication tests.
...
| AT+SNPING4="<ip>",<count>,<size>,<timeout>
| Sends IPv4 ICMP Echo Requests (Ping) using the active PDP context
| Successful replies include:
+SNPING4: <seq>,<ip>,<delay>
followed by OK
If it returns:
* +SNPING4: <seq>,<ip>,-1 → Request timeout (no reply received)
* ERROR → AT syntax error or invalid parameters
* +CME ERROR: <code> → PDP context not active, network issue, or SIM/configuration problem
